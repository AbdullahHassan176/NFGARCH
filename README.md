...
**For reviewers:** Full reproduction steps, prerequisites, pipeline mode (full vs 6-asset), and output locations are in **`docs/REPRODUCTION_FOR_REVIEWERS.md`**. Use `PIPELINE_MODE = "full"` in `scripts/core/config.R` for 13-asset (dissertation) results; use `"optimized"` for the 6-asset run that matches the MDPI article tables.

**Requirements:** R 4.x, Python 3.8+. R packages: xts, zoo, rugarch, PerformanceAnalytics, dplyr, tidyr, stringr, lubridate, openxlsx. Python: `pip install -r environment/requirements.txt`
